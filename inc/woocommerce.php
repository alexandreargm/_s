 ...
 */
function blank_woocommerce_loop_columns() {
	return 3;
}
add_filter( 'loop_shop_columns', 'blank_woocommerce_loop_columns' );
...
if ( ! function_exists( 'blank_woocommerce_product_columns_wrapper' ) ) {
	/**
	 * Product columns wrapper.
	 *
	 * @return  void
	 */
	function blank_woocommerce_product_columns_wrapper() {
		$columns = blank_woocommerce_loop_columns();
		echo '<div class="columns-' . absint( $columns ) . '">';
	}
}
add_action( 'woocommerce_before_shop_loop', 'blank_woocommerce_product_columns_wrapper', 40 );

if ( ! function_exists( 'blank_woocommerce_product_columns_wrapper_close' ) ) {
	/**
	 * Product columns wrapper close.
	 *
	 * @return  void
	 */
	function blank_woocommerce_product_columns_wrapper_close() {
		echo '</div>';
	}
}
add_action( 'woocommerce_after_shop_loop', 'blank_woocommerce_product_columns_wrapper_close', 40 );
...
if ( ! function_exists( 'blank_woocommerce_cart_link' ) ) {
	...
	function blank_woocommerce_cart_link() {
		?>
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'blank' ); ?>">
			<?php
			$item_count_text = sprintf(
				/* translators: number of items in the mini cart. */
				_n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'blank' ),
